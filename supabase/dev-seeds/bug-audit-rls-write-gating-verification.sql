...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    (m_a, 'authenticated', 'authenticated', 'm_a@uniplug-rls.local',
     ...
     jsonb_build_object('role','mentor','date_of_birth','2000-01-01','full_name','Mentor A','university','Test U','course','Test','year','2nd Year'),
     '', '', '', '', now(), now(), '00000000-0000-0000-0000-000000000000'),
    (m_b, 'authenticated', 'authenticated', 'm_b@uniplug-rls.local',
     crypt('pw', gen_salt('bf')), now(),
     '{"provider":"email"}'::jsonb,
     jsonb_build_object('role','mentor','date_of_birth','2000-01-01','full_name','Mentor B','university','Test U','course','Test','year','2nd Year'),
     '', '', '', '', now(), now(), '00000000-0000-0000-0000-000000000000'),
