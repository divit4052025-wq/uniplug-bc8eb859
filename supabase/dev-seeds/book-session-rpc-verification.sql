...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    (m_p, 'authenticated', 'authenticated', 'm_p@a1-bk.local',
     ...
     jsonb_build_object('role','mentor','date_of_birth','2000-01-01','full_name','Pending M','university','T','course','T','year','2nd Year'),
    ...
    (m_r, 'authenticated', 'authenticated', 'm_r@a1-bk.local',
     crypt('pw', gen_salt('bf')), now(), '{"provider":"email"}'::jsonb,
     jsonb_build_object('role','mentor','date_of_birth','2000-01-01','full_name','Rejected M','university','T','course','T','year','2nd Year'),
     '', '', '', '', now(), now(), '00000000-0000-0000-0000-000000000000'),
    (m_x, 'authenticated', 'authenticated', 'm_x@a1-bk.local',
     crypt('pw', gen_salt('bf')), now(), '{"provider":"email"}'::jsonb,
     jsonb_build_object('role','mentor','date_of_birth','2000-01-01','full_name','SelfBook M','university','T','course','T','year','2nd Year'),
     '', '', '', '', now(), now(), '00000000-0000-0000-0000-000000000000'),
