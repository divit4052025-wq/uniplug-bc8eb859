...
function SessionNotesPage() {
  ...
  const toggle = async (note: Note, index: number) => {
    ...
    setNotes((prev) =>
      prev.map((n) =>
        n.id === note.id ? { ...n, completions: { ...n.completions, [index]: next } } : n,
      ),
    );
    await supabase.from("action_point_completions").upsert(
      {
  ...
  const pendingActionPoints = totalActionPoints - completedActionPoints;
    ...
    <div className="min-h-screen bg-[#FFFCFB]">
      <div className="mx-auto max-w-[1100px] px-5 pb-20 pt-8 sm:px-8 md:pt-12">
        ...
        <p className="mt-1 text-[14px] font-light text-[#1A1A1A]/60">
          All your mentor notes and action points in one place.
        </p>
          ...
          {notes.length === 0 ? (
            ...
            notes.map((n) => {
              ...
              return (
                <article
                  key={n.id}
                  className="rounded-2xl border border-[#EDE0DB] bg-[#FFFCFB] p-5 sm:p-6"
                >
                  <div className="flex items-start gap-4">
                    <div
                      aria-hidden
                      className="grid h-11 w-11 shrink-0 place-content-center rounded-full bg-[#EDE0DB] font-display text-[14px] font-semibold text-[#1A1A1A]"
                    >
                      {initials || "M"}
                    </div>
                  ...
                  <div className="mt-5 flex items-center gap-2 border-t border-[#EDE0DB] pt-3">
