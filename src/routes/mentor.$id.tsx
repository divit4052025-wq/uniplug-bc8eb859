...
export const Route = createFileRoute("/mentor/$id")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Book a one-on-one mentorship session with a verified university student.",
      },
    ],
  }),
...
function MentorProfilePage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery<Page>({
    ...
    enabled: authReady,
    queryFn: async () => {
      const { data: profile, error: pErr } = await supabase.rpc("get_mentor_public_profile", {
        _mentor_id: id,
      });
      if (pErr) throw pErr;
      ...
      <div className="min-h-screen bg-[#FFFCFB]">
        ...
        <main className="md:ml-[240px]">
          <div className="mx-auto max-w-2xl px-6 py-24">
            <ErrorBanner
              message="Couldn't load this mentor right now."
      ...
      <div className="min-h-screen bg-[#FFFCFB]">
        ...
        <main className="md:ml-[240px]">
          <div className="mx-auto max-w-2xl px-6 py-24 text-center">
            ...
            <p className="mt-2 text-[14px] text-[#1A1A1A]/60">
              This profile may not be available or hasn't been approved yet.
            </p>
            <Link
              to="/browse"
              className="mt-6 inline-block rounded-full bg-[#C4907F] px-5 py-2.5 text-[13px] font-medium text-[#FFFCFB]"
            >
              Browse mentors
            </Link>
          </div>
          ...
          <div className="mx-auto flex max-w-5xl flex-col gap-8 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[16px] text-[#E8C4B8] sm:text-[18px]">
                  {mentor.university}
                </p>
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {mentor.topics.map((t) => (
                      <span
                        key={t}
                        className="rounded-full bg-[#1A1A1A] px-3.5 py-1.5 text-[12px] font-medium text-[#FFFCFB]"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  return (
                    <article key={r.id} className="rounded-2xl bg-[#FFFCFB] p-5">
                      <div className="flex items-center justify-between">
                        <p className="font-display text-[16px] font-semibold text-[#1A1A1A]">
                          {firstName}
                        </p>
                      ...
                      <div className="mt-1 flex gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star
                            key={i}
                            className={`h-3.5 w-3.5 ${i < r.rating ? "fill-[#C4907F] text-[#C4907F]" : "text-[#1A1A1A]/20"}`}
                          />
                        ))}
                      </div>
                      {r.review && (
                        <p className="mt-3 text-[14px] leading-relaxed text-[#1A1A1A]/80">
                          {r.review}
                        </p>
                      )}
                    </article>
