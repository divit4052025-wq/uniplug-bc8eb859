...
export const Route = createFileRoute("/browse")({
  beforeLoad: () => clientAuthGuard({ signedOutTo: "/student-signup", requireRole: "any" }),
  head: () => ({
    meta: [
      { title: "Browse Plugs — UniPlug" },
      {
        name: "description",
        content: "Discover and filter university student mentors for college admissions guidance.",
      },
...
function BrowsePage() {
  ...
  const filtered = useMemo(() => {
    let list = mentors.filter((m) => {
      if (search && !`${m.name} ${m.university}`.toLowerCase().includes(search.toLowerCase()))
        return false;
      if (countries.length && !m.countries.some((c) => countries.includes(c))) return false;
      if (universityQuery && !m.university.toLowerCase().includes(universityQuery.toLowerCase()))
        return false;
  ...
  const filterPanel = (
    <FilterPanel
      ...
      setSearch={setSearch}
      countries={countries}
      toggleCountry={(v) => toggle(countries, setCountries, v)}
      universityQuery={universityQuery}
      setUniversityQuery={setUniversityQuery}
      courses={courses}
      toggleCourse={(v) => toggle(courses, setCourses, v)}
      years={years}
      toggleYear={(v) => toggle(years, setYears, v)}
      sort={sort}
      setSort={setSort}
      clearAll={clearAll}
    />
        ...
        <div className="flex">
          ...
          <section className="flex-1 px-5 pb-28 pt-6 sm:px-8 md:px-10 md:pb-12 md:pt-10">
            <div className="mb-6 hidden md:block">
              <h1 className="font-display text-[28px] font-semibold tracking-tight text-[#1A1A1A]">
                Browse Plugs
              </h1>
              <p className="mt-1 text-[14px] text-[#1A1A1A]/60">
                Find a mentor who's been exactly where you want to go.
              </p>
            </div>
            {isError ? (
              <ErrorBanner
...
function FilterPanel({
  ...
  setUniversityQuery: (v: string) => void;
  courses: string[];
  toggleCourse: (v: string) => void;
  years: string[];
  ...
  return (
    <div className="flex flex-col gap-5 p-6">
      <h2 className="font-display text-[20px] font-semibold tracking-tight text-[#1A1A1A]">
        Find Your Plug
      </h2>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-t border-[#EDE0DB] pt-4">
      <h3 className="mb-3 text-[12px] font-semibold uppercase tracking-wider text-[#1A1A1A]/70">
        {title}
      </h3>
      {children}
    </div>
    ...
    <article className="group flex flex-col rounded-2xl border border-[#E8C4B8] bg-[#EDE0DB] p-5 transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_-20px_rgba(26,26,26,0.25)]">
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {mentor.topics.map((t) => (
          <span
            key={t}
            className="rounded-full bg-[#1A1A1A] px-2.5 py-1 text-[11px] font-medium text-[#FFFCFB]"
          >
            {t}
          </span>
        ))}
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display text-[18px] font-bold leading-tight text-[#1A1A1A]">
            {mentor.name}
          </h3>
        </div>
