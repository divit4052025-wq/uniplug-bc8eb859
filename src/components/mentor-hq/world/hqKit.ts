...
export const THEMES: Record<ThemeName, Pal> = {
  ...
  mix: {
    // EXACT authored palette from the prototype (hq3d/kit.js, three r128).
    // Reverted from the prior eyeball-lightened drift; the look is corrected via
    // the lighting-model fix in hqScene.ts, not by paling these albedos.
...
export const TIME: Record<TimeName, TimePal> = {
  golden: {
    // EXACT authored values from the prototype (hq3d/kit.js, three r128).
    // Do NOT eyeball-tune these. The r128→r185 lighting-model gap is corrected
    // by re-applying the legacy `irradiance *= PI` factor to the indirect lights
    // in hqScene.ts applyTime(), NOT by drifting these numbers.
    label: "Golden hour",
    exposure: 0.78,
    sky: [
      [0.0, "#8FB0CC"],
